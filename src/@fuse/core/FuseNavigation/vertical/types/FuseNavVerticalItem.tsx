...
const Root = styled(ListItemButton)<ListItemButtonStyleProps>(({ theme, ...props }) => ({
	...
	'&.active': {
		color: theme.vars.palette.text.primary,
		backgroundColor:
			theme.palette.mode === 'light' ? 'rgba(255, 90, 42, .05)!important' : 'rgba(255, 90, 42, 0.1)!important',
	...
	const memoizedContent = useMemo(
		() => (
			...
			>
				{item.icon && (
					<FuseSvgIcon
						className={clsx('fuse-list-item-icon shrink-0 !min-w-[16px] !w-[16px]', item.iconClass)}
						color="action"
					>
						{item.icon}
					</FuseSvgIcon>
