...
const selectVisitorsVsPageViews = selectWidget<VisitorsVsPageViewsType>('visitorsVsPageViews');
...
function VisitorsVsPageViewsWidget() {
	...
	const widget = useAppSelector(selectVisitorsVsPageViews);
	...
	const chartOptions = useMemo<ApexOptions>(
		() => ({
			...
			tooltip: { followCursor: true, theme: 'dark', x: { format: 'MMM dd, yyyy' } },
			xaxis: {
		...
		[theme.palette.primary.light, theme.palette.primary.dark, theme.palette.text.secondary, theme.palette.divider]
	);

	if (!widget) {
		return (
			<Paper className="flex flex-col flex-auto shadow-sm rounded-xl overflow-hidden p-6">
...
export default memo(VisitorsVsPageViewsWidget);
