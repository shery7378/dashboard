...
/**
 * Visits widget.
 * Uses memoized chart options and a stable Redux selector.
 */
function VisitsWidget() {
	const theme = useTheme();
	const widget = useAppSelector(selectVisits);
	...
	const chartOptions = useMemo<ApexOptions>(
		() => ({
			...
			fill: { colors: [theme.palette.error.light], opacity: 0.5 },
			stroke: { curve: 'smooth' },
	...
	if (!widget) {
		return (
			<Paper className="flex flex-col flex-auto shadow-sm rounded-xl overflow-hidden p-4">
				...
				<Skeleton variant="text" width={60} height={80} sx={{ mt: 1 }} />
...
export default memo(VisitsWidget);
