...
function VisitorsOverviewWidget() {
	...
	const memoChartOptions: ApexOptions = useMemo(
		...
		[
			contrastTheme.palette.primary.contrastText,
			contrastTheme.palette.primary.contrastTextChannel,
			contrastTheme.palette.secondary.dark,
			contrastTheme.palette.secondary.light,
			contrastTheme.palette.secondary.main
		]
	);

	const memoSeries = useMemo(
		() => (currentRange && series ? [...(series[currentRange] || [])] : []),
		[currentRange, series]
	);
