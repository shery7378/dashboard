 ...
 * Uses memoized chart options and a stable Redux selector to avoid re-renders.
 */
function ConversionsWidget() {
	const theme = useTheme();
	const widget = useAppSelector(selectConversions);
	const series = widget?.series;
	const amount = widget?.amount;
	const labels = widget?.labels;

	// Memoize chart options so they're only rebuilt when the theme changes
	const chartOptions = useMemo<ApexOptions>(
		() => ({
			...
			fill: { colors: [theme.palette.secondary.light], opacity: 0.5 },
			stroke: { curve: 'smooth' },
...
export default memo(ConversionsWidget);
