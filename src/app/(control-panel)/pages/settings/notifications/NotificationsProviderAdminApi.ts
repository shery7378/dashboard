...
const NotificationsProviderAdminApi = api
  ...
  .injectEndpoints({
    endpoints: (build) => ({
      ...
      updateAdminNotificationProviderSettings: build.mutation<UpdateAdminNotificationProviderSettingsResponse, UpdateAdminNotificationProviderSettingsArg>({
      ...
      sendNotificationTest: build.mutation<SendNotificationTestResponse, SendNotificationTestArg | void>({
        query: (body) => ({ url: `/api/admin/notification-provider-settings/test`, method: 'POST', body: body ?? {} }),
      }),
    }),
...
export type UpdateAdminNotificationProviderSettingsArg = Partial<Pick<NotificationProviderSettings, 'email_from_name' | 'email_from_address' | 'sms_provider' | 'sms_api_key' | 'sms_api_secret' | 'sms_sender_id' | 'mail_mailer' | 'mail_scheme' | 'mail_host' | 'mail_port' | 'mail_username' | 'mail_password' | 'mail_from_address' | 'mail_from_name'>>;
