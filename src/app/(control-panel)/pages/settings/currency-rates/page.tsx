...
import {
  useGetAdminCurrencyRatesQuery,
  useUpdateAdminCurrencyRatesMutation,
} from './CurrencyRatesAdminApi';
...
export default function CurrencyRatesSettingsPage() {
  const {
    ...
    error,
    refetch,
  } = useGetAdminCurrencyRatesQuery(undefined, {
  ...
  const hasRates = useMemo(
    () => localRates && Object.keys(localRates).length > 0,
    [localRates],
  );
            ...
            <CardHeader
              ...
              subheader={
                <Typography variant="body2" sx={{ color: 'rgba(148,163,184,0.9)' }}>
                  Example: if default currency is {defaultCurrency} and you set USD = 1.25, then
                  1 {defaultCurrency} = 1.25 USD.
                </Typography>
              }
              sx={{
                pb: 0,
            ...
            <CardContent sx={{ pt: 3 }}>
              ...
              {!isFetching && !hasRates && (
                <Alert severity="info" sx={{ backgroundColor: 'rgba(15,23,42,0.9)', color: 'white' }}>
                  No currencies configured yet. Please add supported currencies and a default currency on the
                  Currency Settings page, then reload this page.
                </Alert>
              )}
