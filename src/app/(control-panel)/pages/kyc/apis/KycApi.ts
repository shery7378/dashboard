  ...
  .injectEndpoints({
    endpoints: (build) => ({
      ...
      listKycSubmissions: build.query<
        { status: number; data: { id: number; vendor_id: number; vendor_name: string; status: string; submitted_at: string }[] },
      ...
      rejectKyc: build.mutation<
        { status: number; message: string },
        { submissionId: number; reason?: string }
      >({
      ...
      getKycDocuments: build.query<
        { status: number; data: Partial<Record<'id_front' | 'id_back' | 'business_license' | 'bank_statement', { filename: string; url: string }>> },
        { submissionId: number }
      >({
        query: ({ submissionId }) => ({
