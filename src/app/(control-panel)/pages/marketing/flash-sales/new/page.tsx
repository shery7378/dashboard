...
import { Container, Grid, Typography, Paper, TextField, MenuItem, FormControlLabel, Switch, Divider, Stack, Button, Alert, Autocomplete, Chip, CircularProgress, Box, InputAdornment, Zoom } from "@mui/material";
import { KeyboardArrowLeft, AccessTime, FlashOn, Percent, AttachMoney, DateRange, Storefront, ShoppingBag, ArrowForward } from "@mui/icons-material";
        ...
        <Stack spacing={4}>
          ...
          <Box>
            <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1, opacity: 0.7 }}>
              <Typography variant="body2" sx={{ fontWeight: 500 }}>Marketing</Typography>
              <Typography variant="body2">/</Typography>
              <Typography variant="body2" sx={{ fontWeight: 500 }}>Flash Sales</Typography>
            </Stack>
          ...
          >
            <Paper
              ...
              sx={{
                p: 3,
                borderRadius: 4,
                background: (theme) => `linear-gradient(135deg, ${theme.palette.info.light}15 0%, ${theme.palette.info.main}10 100%)`,
                border: '1px solid',
            ...
            >
              ...
              <Box>
                ...
                <Typography variant="body2" color="info.dark" sx={{ opacity: 0.8 }}>
                  All sales are evaluated in <strong>UTC</strong>.
                  (Local: {localTimeLabel} • UTC: {utcTimeLabel})
                </Typography>
              </Box>
            </Paper>
          </motion.div>
          ...
          <Box component="form" onSubmit={submit}>
            <Grid container spacing={4}>
              ...
              <Grid item xs={12} md={7}>
                <Stack spacing={4}>
                  <Paper elevation={0} sx={{ p: 4, borderRadius: 5, border: '1px solid', borderColor: 'divider' }}>
                    <Stack spacing={3}>
                      <Stack direction="row" spacing={1} alignItems="center">
                        ...
                        <Typography variant="h6" fontWeight={700}>Sale Basics</Typography>
                      </Stack>
                      ...
                      <Grid container spacing={2}>
                            ...
                            InputProps={{
                              startAdornment: (
                                <InputAdornment position="start">
                                  {form.discount_type === 'percent' ? <Percent fontSize="small" color="action" /> : <Typography variant="h6" color="action" sx={{ fontSize: '1.2rem', fontWeight: 600 }}>{currencyConfig.symbol}</Typography>}
                                </InputAdornment>
                              ),
                            }}
                          >
                        ...
                        <Grid item xs={12} sm={6}>
                          <TextField
                            ...
                            helperText={form.discount_type === 'percent' ? 'Percentage off price' : 'Amount off price'}
                      ...
                      <Stack direction="row" spacing={1} alignItems="center">
                        <Box sx={{ p: 1, bgcolor: 'secondary.main', borderRadius: 2, display: 'flex', color: 'white' }}>
                  ...
                  <Paper elevation={0} sx={{ p: 4, borderRadius: 5, border: '1px solid', borderColor: 'divider' }}>
                    <Stack spacing={3}>
                      <Stack direction="row" spacing={1} alignItems="center">
                        <Box sx={{ p: 1, bgcolor: 'warning.main', borderRadius: 2, display: 'flex', color: 'white' }}>
                          <Storefront fontSize="small" />
                        </Box>
                        <Typography variant="h6" fontWeight={700}>Inventory Selection</Typography>
                      </Stack>
                      ...
                      <Autocomplete
                        ...
                        renderInput={(params) => (
                          <TextField
                            ...
                            variant="outlined"
                            sx={{ '& .MuiOutlinedInput-root': { borderRadius: 3 } }}
                            helperText={productsError ? productsError : (!productsLoading && productOptions.length === 0 ? (storeId ? 'No products found for the selected store' : 'Select a store to load products') : (`Choose the products you want to feature in this sale`))}
                            InputProps={{
                              ...params.InputProps,
                              startAdornment: (
                                <>
              ...
              <Grid item xs={12} md={5}>
                <Stack spacing={4} sx={{ position: { md: 'sticky' }, top: 24 }}>
                  <Paper
                    elevation={0}
                    sx={{
                      p: 4,
                  ...
                  >
                    <Stack spacing={3}>
                      <Typography variant="h6" fontWeight={700}>Event Status</Typography>

                      <Box
                        sx={{
                          p: 3,
                          borderRadius: 4,
                          bgcolor: form.is_active ? 'success.light' : 'action.disabledBackground',
                      ...
                      >
                        <Stack spacing={1} alignItems="center">
                          <Typography variant="h4" fontWeight={800}>{form.is_active ? 'ACTIVE' : 'INACTIVE'}</Typography>
                          <Typography variant="caption" sx={{ textTransform: 'uppercase', letterSpacing: 1 }}>
                            {form.is_active ? 'Sale will launch as scheduled' : 'Sale is currently hidden'}
                          </Typography>
                        </Stack>
                      </Box>
                      ...
                      <Box>
                        <Typography variant="subtitle2" gutterBottom fontWeight={700}>Promotion Summary</Typography>
                        <Stack spacing={1.5} sx={{ mt: 2 }}>
                          <Stack direction="row" justifyContent="space-between">
                            <Typography variant="body2" color="text.secondary">Discount Type</Typography>
                            <Typography variant="body2" fontWeight={600}>{form.discount_type === 'percent' ? 'Percentage' : 'Fixed Amount'}</Typography>
                          </Stack>
                          <Stack direction="row" justifyContent="space-between">
                            <Typography variant="body2" color="text.secondary">Discount Value</Typography>
                            <Typography variant="body2" fontWeight={600} color="primary.main">
                              {form.discount_type === 'percent' ? `${form.discount_value}%` : `${currencyConfig.symbol}${form.discount_value}`}
                            </Typography>
                          </Stack>
                          <Stack direction="row" justifyContent="space-between">
                            <Typography variant="body2" color="text.secondary">Target Products</Typography>
                            <Typography variant="body2" fontWeight={600}>{form.product_ids?.length || 0} items</Typography>
                          </Stack>
                        </Stack>
                      </Box>
                    </Stack>
