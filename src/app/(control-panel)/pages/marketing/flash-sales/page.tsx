          ...
          products: (item.products || []).map((product: any) => {
            ...
            if (item.discount_type === 'percent') {
              discountedPrice = Math.max(
                0,
                originalPrice * (1 - Number(item.discount_value) / 100)
              );
            } else if (item.discount_type === 'fixed') {
              discountedPrice = Math.max(
                0,
                originalPrice - Number(item.discount_value)
              );
            }
    ...
    <Box sx={{
      ...
      <Container maxWidth="lg">
        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 3 }}>
          <Box>
            <Typography variant="h4" fontWeight={700}>Flash Sales</Typography>
            <Typography variant="body2" color="text.secondary">
              Schedule time-bound discounts for selected products.
            </Typography>
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.5 }}>
              Flash sale start/end times are evaluated on the server in <strong>UTC</strong>. Dates shown here use your browser&apos;s local time zone.
            </Typography>
          </Box>
          <Button component={Link as any} href="/pages/marketing/flash-sales/new" variant="contained">New Flash Sale</Button>
        </Stack>
        {loading && <LinearProgress />}
        {error && (
          <Paper sx={{ p: 2, my: 2, bgcolor: 'error.light', color: 'error.contrastText', borderRadius: 2 }}>{error}</Paper>
        )}
        {!loading && items?.length === 0 && (
          <Paper sx={{ p: 4, textAlign: 'center', borderRadius: 3, border: (t) => `1px dashed ${t.palette.divider}` }}>
            <Typography variant="h6" fontWeight={600}>No flash sales yet</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>Create your first flash sale to boost conversions.</Typography>
            <Button component={Link as any} href="/pages/marketing/flash-sales/new" sx={{ mt: 2 }} variant="outlined">Create flash sale</Button>
          </Paper>
        )}
        {items?.length > 0 && (
          <TableContainer component={Paper} sx={{ borderRadius: 3, overflow: 'hidden', border: (t) => `1px solid ${t.palette.divider}` }}>
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  return (
                    <React.Fragment key={x.id}>
                      <TableRow hover>
                        ...
                        <TableCell>
                          <Typography variant="body2" fontWeight={600}>{x.name}</Typography>
                          {x.is_active ? (
                            <Chip size="small" label="Active" color="success" sx={{ mt: 0.5, height: 20 }} />
                          ) : (
                            <Chip size="small" label="Inactive" color="default" sx={{ mt: 0.5, height: 20 }} />
                          )}
                        </TableCell>
                        <TableCell>
                          <Chip
                        ...
                        <TableCell align="right">
                          <Stack direction="row" spacing={1} justifyContent="flex-end">
                            <Button size="small" color="error" variant="outlined" onClick={() => remove(x.id)}>Delete</Button>
                          </Stack>
                        </TableCell>
                      </TableRow>
