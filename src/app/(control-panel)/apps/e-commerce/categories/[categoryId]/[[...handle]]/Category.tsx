...
function Category() {
	...
	const initialCategoryType =
		categoryId === 'new' ? 'child' : category?.data?.parent_id === null ? 'parent' : 'child';
	...
	const methods = useForm({
		...
		defaultValues: {
			...{
				...
				parent_id: createDefaultCategory().parent_id !== null ? Number(createDefaultCategory().parent_id) : null
			}
	...
	useEffect(() => {
		if (categoryId === 'new') {
			...
			methods.reset({
				...sanitized,
				parent_id:
					sanitized.parent_id !== null && sanitized.parent_id !== undefined
