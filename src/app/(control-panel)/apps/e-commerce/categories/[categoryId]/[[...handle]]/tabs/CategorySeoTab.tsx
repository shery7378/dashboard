...
export default function CategorySeoTab() {
					...
					render={({ field }) => (
						<TextField
							...
							helperText={`${metaTitle.length}/60 ${
								errors?.meta_title?.message ? ' - ' + errors.meta_title.message : ''
							}`}
						/>
					)}
				/>
				...
				<Controller
					...
					render={({ field }) => (
						<TextField
							...
							helperText={`${metaDesc.length}/160 ${
								errors?.meta_description?.message ? ' - ' + errors.meta_description.message : ''
							}`}
						/>
					)}
				/>
