...
function LiveSellingSessionDialog({ open, onClose, session }: LiveSellingSessionDialogProps) {
	...
	const fetchProducts = async () => {
		try {
			...
			enqueueSnackbar(err.response?.data?.message || 'Failed to load products. Please try again.', {
				variant: 'error'
			});
			setProducts([]);
		}
		...
		<LocalizationProvider dateAdapter={AdapterDateFns}>
			...
			>
				...
				>
					...
					>
						{session?.id
							? 'Update your live selling session details'
							: 'Fill in the details below to create your live selling session'}
					</Box>
				</Box>
							...
							renderInput={(params) => (
								<TextField
									...
									placeholder={
										products.length === 0
											? 'Loading products...'
											: 'Select products to feature in this session'
									}
									helperText={
										products.length === 0
