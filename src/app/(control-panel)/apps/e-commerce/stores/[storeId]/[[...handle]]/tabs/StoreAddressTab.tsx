            ...
            geocoder.geocode({ location: pos }, (results, status) => {
                if (status === "OK" && results?.length > 0) {
                    ...
                    for (const result of results) {
                        ...
                        const hasStreetInfo = components.some(c => 
                    ...
                    return (
                        <Autocomplete
                            ...
                            onPlaceChanged={() => {
                                ...
                                if (place && place.geometry && place.geometry.location && place.address_components && place.address_components.length > 0) {
                                    ...
                                    if (place.name && place.name.trim() && !place.name.match(/^[A-Z0-9]{6}\+[A-Z0-9]{2,3}$/i)) {
                                        const nameLower = place.name.toLowerCase();
                                        if (nameLower.includes("street") || nameLower.includes("road") || nameLower.includes("avenue") ||
                                            nameLower.includes("lane") || nameLower.includes("drive") || /\d/.test(place.name)) {
