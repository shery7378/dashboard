...
/**
 * SEO tab for a store: meta title, meta description, and meta keywords.
 */
export default function StoreSeoTab() {
	...
	/**
	 * Displays a field label with a tooltip icon.
	 */
	...
	return (
		<div className="grid gap-2 md:grid-cols-1">
				...
				<Controller
					...
					render={({ field }) => (
						<TextField
							...
							helperText={`${metaTitle.length}/60${errors.meta_title ? ` - ${errors.meta_title.message}` : ''}`}
						/>
					)}
				/>
			</div>
				...
				<Controller
					...
					render={({ field }) => (
						<TextField
							...
							helperText={`${metaDesc.length}/160${errors.meta_description ? ` - ${errors.meta_description.message}` : ''}`}
						/>
					)}
				/>
			</div>
			...
			<div>
				<FieldLabel
					label="Meta Keywords"
					tooltip="Comma-separated keywords to improve SEO visibility (e.g. 'shoes, sneakers, fashion')."
				/>
				<Controller
					name="meta_keywords"
					control={control}
