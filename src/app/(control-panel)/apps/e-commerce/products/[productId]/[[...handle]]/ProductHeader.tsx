...
function ProductHeader() {
	...
	const [createProduct, { isLoading: isCreating }] = useCreateECommerceProductMutation();
	const [saveProduct, { isLoading: isSaving }] = useUpdateECommerceProductMutation();
	const [removeProduct, { isLoading: isDeleting }] = useDeleteECommerceProductMutation();
	...
	function transformFormValues(values: EcommerceProduct): any {
		...
		const transformed: any = {
			...
			...(values.subcategory &&
				Array.isArray(values.subcategory) &&
				values.subcategory.length > 0 && { subcategory: values.subcategory }),
			...
			attributes: (() => {
				...
				if (values.attributes && Array.isArray(values.attributes) && values.attributes.length > 0) {
					...
					if (
						firstAttr &&
						typeof firstAttr === 'object' &&
						'attribute_id' in firstAttr &&
						'values' in firstAttr
					) {
			...
			shipping_charge_regular:
				parseFloat(String(values.shipping_charge_regular ?? (values as any).shippingChargeRegular ?? 0)) || 0,
			shipping_charge_same_day:
				parseFloat(String(values.shipping_charge_same_day ?? (values as any).shippingChargeSameDay ?? 0)) || 0,
				...
				if (variant && typeof variant === 'object') {
					const variantPrice =
						variant.price !== undefined && variant.price !== null
							...
							: 0;
					const variantQty =
						variant.qty !== undefined && variant.qty !== null
					...
					const finalPrice =
						variant.price_tax_excl !== undefined && variant.price_tax_excl !== null
					...
					return {
						...
						price_tax_incl:
							variant.price_tax_incl !== undefined && variant.price_tax_incl !== null
								? parseFloat(String(variant.price_tax_incl)) || finalPrice
								: finalPrice, // Database column (if exists)
						special_price:
							variant.special_price !== undefined && variant.special_price !== null
								? parseFloat(String(variant.special_price)) || 0
								: 0,
						compared_price:
							variant.compared_price !== undefined && variant.compared_price !== null
						...
						manage_stock:
							variant.manage_stock !== undefined
								? Boolean(variant.manage_stock)
								: variantQty > 0
									? true
									: false,
						in_stock:
							variant.in_stock !== undefined ? Boolean(variant.in_stock) : variantQty > 0 ? true : false,
						...
						position:
							variant.position !== undefined
								? parseInt(String(variant.position)) || index + 1
								: index + 1,
			...
			transformed.variants = values.variants.map((variant: any, index: number) => {
				if (variant && typeof variant === 'object') {
					...
					return {
						...
						is_default: variant.is_default !== undefined ? Boolean(variant.is_default) : false,
						position:
							variant.position !== undefined ? parseInt(String(variant.position)) || index + 1 : index + 1
					};
				}
		...
		saveProduct(transformedData)
			...
			.catch((error) => {
				...
				if (!errorMessage || errorMessage === 'Unknown error occurred') {
					errorMessage = error?.message || error?.error || `HTTP ${error?.status || 'Unknown'} Error`;
				}
	...
	function handleSaveProduct() {
		...
		methods.trigger().then((isFormValid) => {
			if (!isFormValid) {
				...
				if (errors.name) {
					enqueueSnackbar(errors.name.message || 'Product name is required (minimum 5 characters)', {
						variant: 'error'
					});
					return;
				}
				...
				if (errors.description) {
					enqueueSnackbar(errors.description.message || 'Description is required (minimum 10 characters)', {
						variant: 'error'
					});
					return;
				}
				...
				if (errors.gallery_images) {
					enqueueSnackbar(errors.gallery_images.message || 'At least one image is required', {
						variant: 'error'
					});
					return;
				}
				...
				if (firstError) {
					enqueueSnackbar(firstError.message || t('please_fill_all_required_fields'), { variant: 'error' });
				} else {
	...
	function proceedWithSave(values: EcommerceProduct) {
		...
		saveProduct(transformedData)
			...
			.catch((error) => {
				...
				enqueueSnackbar(`${t('failed_to_update_product')}: ${errorMessage}`, {
					variant: 'error',
		...
		if (isDraft) {
			...
			methods.trigger().then((isFormValid) => {
				if (!isFormValid) {
					...
					if (errors.store_id) {
						enqueueSnackbar(
							errors.store_id.message ||
								'Store is required. You must have a store before creating products.',
							{ variant: 'error', autoHideDuration: 6000 }
						);
						return;
					}
					...
					if (firstError) {
						enqueueSnackbar(firstError.message || t('please_fill_all_required_fields'), {
							variant: 'error'
						});
					} else {
						enqueueSnackbar(t('please_fill_all_required_fields'), { variant: 'error' });
	...
	function handleRemoveConfirmed() {
		removeProduct(productId)
			...
			.catch((error) => {
				enqueueSnackbar(`${t('failed_to_delete_product')} ${error.data?.error ?? ''}`, { variant: 'error' });
			})
							...
							<img
								...
								src={
									(Array.isArray(gallery_images)
										? gallery_images.find((img: any) => img && img.is_featured)
										: null
									)?.url ?? '/assets/images/apps/ecommerce/product-image-placeholder.png'
								}
								alt={name}
							/>
						) : (
			...
			>
				{productId !== 'new' ? (
					...
					<Button
						...
						title={
							!isValid
								? 'Please fill all required fields (Name, Description, Category, Subcategory, Images)'
								: ''
						}
						startIcon={
							isCreating ? (
