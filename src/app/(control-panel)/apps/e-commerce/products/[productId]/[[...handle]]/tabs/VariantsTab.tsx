...
const formatVariant = (variant, attributeOptions) => {
	...
	variantAttributes.forEach((attr) => {
		...
		if (!attributeMap.has(key)) {
			...
			const mergedOptions = Array.from(new Set([...(existing.options || []), ...(match?.options || [])]));
	...
	return {
		...
		extraVariantFields: mappedAttributes.reduce((acc, attr) => {
			acc[attr.name] =
				attr.value ||
				(attr.type === 'number' ? 0 : attr.type === 'select' ? (attr.name === 'Color' ? [] : '') : '');
			return acc;
		}, {}),
...
function VariantsTab() {
	...
	useEffect(() => {
		...
		if (
			...
			didSyncVariants.current ||
			isEqual(productVariants, prevProductVariants.current)
		) {
							...
							<Controller
								...
								render={({ field: { onChange, value } }) => {
									const handleRemoveField = (idToRemove) => {
										...
										if (attributeName) {
											setValue(
												`variants[${index}].extraVariantFields.${attributeName}`,
												undefined,
												{
										...
										const updatedFields = (dynamicFieldsMap[index] || []).filter(
											(f) => f.id !== idToRemove
										);
											...
											<Autocomplete
												...
												onChange={(event, newValue) => {
													...
													setVariants((prev) => {
														...
														console.log(
															'Updated variants after attribute change:',
															newVariants
														);
														return newVariants;
													});
																...
																<div className="flex-1">
																	...
																	field.name === 'Color' ? (
																		<Autocomplete
																			...
																			onChange={(e, newValue) => {
																				console.log(
																					`Color Autocomplete onChange: ${field.name}`,
																					newValue
																				);
																		...
																		<Autocomplete
																			...
																			onChange={(event, newValue) => {
																				console.log(
																					`Select Autocomplete onChange: ${field.name}`,
																					newValue
																				);
																				fieldProps.onChange(newValue);
																			}}
