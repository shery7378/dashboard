...
function ImportProductModal({ open, onClose, onProductSelect, mode = 'import' }: ImportProductModalProps) {
	...
	const handleImport = async (productId: string | number | undefined) => {
		...
		try {
			...
			const errorMessage =
				error?.data?.message || error?.data?.error || error?.message || t('failed_to_import_product');
						...
						>
							<AnimatePresence>
								{products.map((product: EcommerceProduct, index: number) => {
									...
									const priceTaxIncl = parseFloat(
										(product.price_tax_incl || product.price || 0).toString()
									);
									...
									const productName =
										...
										(product.sku
											? product.sku.replace(/-/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase())
											: null) ||
										`Product #${productId}`;
														...
														<Button
															...
															onClick={() => {
																if (!productId) {
																	...
																	enqueueSnackbar(
																		'Product ID is missing. Cannot import.',
																		{
																			variant: 'error',
																			anchorOrigin: {
																				vertical: 'top',
