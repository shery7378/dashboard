...
interface PaymentMethodDialogProps {
    open: boolean;
    onClose: () => void;
    onConfirm: (paymentMethod: 'instant' | 'credit', quantity: number, creditDays?: number, paymentIntentId?: string) => void;
    productId: string;
    supplierId?: string;
    productPrice?: number;
    isLoading?: boolean;
    ...
    const handleStripePayment = async () => {
        ...
        try {
            ...
            if (typeof window !== 'undefined') {
                ...
                if (!stripeKey) {
                    throw new Error('Stripe publishable key not configured. Please set NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY in your environment variables.');
                }
                                            ...
                                            label={
                                                <Box>
                                                    <Box display="flex" alignItems="center" gap={1}>
                                                        ...
                                                        {method.method === 'credit' && method.available_credit !== undefined && (
                                                            <Chip
                                                                label={`Available: £${Number(method.available_credit).toFixed(2)}`}
                                                                size="small"
                                                                color={Number(method.available_credit) >= totalAmount ? 'success' : 'warning'}
                                                            />
                                                        )}
                                                    </Box>
                                ...
                                {selectedMethod.available_credit !== undefined && (
                                    <Alert severity="info" sx={{ mt: 2 }}>
                                        <Typography variant="body2">
                                            <strong>Credit Limit:</strong> £{selectedMethod.credit_limit ? Number(selectedMethod.credit_limit).toFixed(2) : '0.00'}
                                            <br />
                                            <strong>Used Credit:</strong> £{selectedMethod.used_credit ? Number(selectedMethod.used_credit).toFixed(2) : '0.00'}
                                            <br />
                                            <strong>Available Credit:</strong> £{Number(selectedMethod.available_credit).toFixed(2)}
                                        </Typography>
                                    </Alert>
                                )}
                            </Box>
            ...
            <DialogActions sx={{ px: 3, py: 2, borderTop: 1, borderColor: 'divider' }}>
                ...
                >
                    {isLoading || processingPayment 
                        ...
                        : (paymentMethod === 'instant' ? 'Pay & Import' : 'Confirm Import')}
                </Button>
            </DialogActions>
        </Dialog>
    );
