...
function applyPermissionsAndPrune(
  ...
  return items
    .map((item) => {
      ...
      const children = item.children
        ? applyPermissionsAndPrune(item.children, roles)
        : undefined;

      const isBranch = item.type === 'collapse' || item.type === 'group';
      const hasPermission =
        isBranch ? (permitted && (children ? children.length > 0 : true)) : permitted;
