...
const AnalyticsApi = api.enhanceEndpoints({ addTagTypes }).injectEndpoints({
	endpoints: (build) => ({
		...
		getSalesHeatmap: build.query<
			{
				status: number;
				data: { store_id: number; latitude: number; longitude: number; total_sales: number; orders: number }[];
			},
		...
		getTopProducts: build.query<
			{
				status: number;
				data: { product_id: number; product_name: string; total_sales: number; orders: number }[];
			},
			{ from?: string; to?: string; limit?: number }
		>({
...
export const { useGetSalesQuery, useGetSalesHeatmapQuery, useGetVendorPerformanceQuery, useGetTopProductsQuery } =
	AnalyticsApi;
