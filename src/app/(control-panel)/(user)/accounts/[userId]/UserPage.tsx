...
export const updateProfileSchema = z
	...
	.refine(
		(data) => !data.password || data.password === data.confirm_password, // only check match if password entered
		{
			message: 'Passwords do not match',
			path: ['confirm_password']
		}
...
function UserPage() {
	...
	// ---------------- Submit handler ----------------
	const onSubmit = async (formData: z.infer<typeof createProfileSchema> | z.infer<typeof updateProfileSchema>) => {
		setFormError(null);
		try {
			...
			const displayMsg =
				typeof errorMsg === 'string' ? errorMsg : 'Validation failed. Please check all required fields.';
