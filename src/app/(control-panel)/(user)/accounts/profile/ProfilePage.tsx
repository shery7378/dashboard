...
import { useGetProfileQuery, useCreateProfileMutation, useUpdateProfileMutation, useUpdatePasswordMutation } from '@/app/(control-panel)/(user)/accounts/apis/ProfileApi';
...
function ProfilePage() {
    ...
    const onProfileSubmit = async (data: z.infer<ReturnType<typeof schema>>, tab: string) => {
        ...
        const filteredData = Object.fromEntries(
            Object.entries(data).filter(([key]) => tabFields[tab]?.includes(key))
        );
    ...
    return (
        <>
            <FormProvider {...methods}>
                <Root
                    header={<ProfileHeader onSubmit={handleSubmit(handleFormSubmit)} tabValue={tabValue} />}
                    content={
                        <motion.div
                        ...
                        >
                            ...
                            <FuseTabs value={tabValue} onChange={handleTabChange}>
                                <FuseTab value="change-password" label="Change Password" />
