	...
	try {
		...
		if (!apiKey || !apiKey.startsWith('sk_')) {
			return NextResponse.json(
				{
					error: 'persona_config_error',
					detail: 'PERSONA_API_KEY missing or malformed. Ensure no inline comments and value starts with sk_.'
				},
				{ status: 500 }
			);
		}
		...
		if (!templateId) {
			return NextResponse.json(
				{ error: 'persona_config_error', detail: `Missing ${type.toUpperCase()} template id.` },
				{ status: 500 }
			);
		}
