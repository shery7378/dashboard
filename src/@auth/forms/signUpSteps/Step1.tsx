...
export default function Step1({ control, errors, email, isSendingCode, countdown, handleSendCode }: Step1Props) {
		...
		<div className="w-full px-2 sm:px-4">
			...
			<div className="relative flex items-center justify-center mb-8 mt-2">
				<img
					...
					alt="MultiKonnect"
					className="h-10 w-auto object-contain cursor-pointer brightness-0 invert"
				/>
			</div>
			...
			<AuthButton
				...
				className="h-12 py-3"
			>
				{isSendingCode ? 'Sending...' : countdown > 0 ? `Resend Code (${countdown}s)` : 'Get Code'}
			</AuthButton>
		</div>
	);
}
